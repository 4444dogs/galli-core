...
void SolanaTxManager::OnGetLatestBlockhashHardware(
  ...
  // auto tx_data = meta->tx()->ToSolanaTxData(); // TODO - is this necessary?
  // TODO(nvonpentz) Pass bytes not base64 encoded string
...
void SolanaTxManager::ProcessSolanaHardwareSignature(
  ...
  // TODO(nvonpentz) Should we raise an error (which?) or send ""? Elsewhere we
  // send ""
