...
class RewardsDOMHandler
 ...
 private:
  ...
  void OnGetAdsHistory(const base::Value::List& history);
...
void RewardsDOMHandler::OnGetAdsHistory(const base::Value::List& ads_history) {
  if (!IsJavascriptAllowed()) {
    return;
  }

  CallJavascriptFunction("brave_rewards.adsHistory",
                         base::Value(ads_history.Clone()));
}
